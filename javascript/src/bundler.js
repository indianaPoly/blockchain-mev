...
export class Bundler {
    ...
    setup = async () => {
        ...
        this.flashbots = await FlashbotsBundleProvider.create(this.provider, this.signer, PRIVATE_RELAY);
    };
    ...
    transferOutTx = async (token, maxPriorityFeePerGas, maxFeePerGas) => {
        let calldata = this.bot.interface.encodeFunctionData('recoverToken', [token]);
        return {
            ...(await this._common_fields()),
            to: this.bot.address,
            data: calldata,
    ...
    approveTx = async (router, tokens, force, maxPriorityFeePerGas, maxFeePerGas) => {
        let calldata = this.bot.interface.encodeFunctionData('approveRouter', [router, tokens, force]);
        return {
            ...(await this._common_fields()),
            to: this.bot.address,
            data: calldata,
