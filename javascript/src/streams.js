...
export const parseUniswapV3Event = (event) => {
  ...
  if (
    ...
    event.topics[0] ===
    ethers.utils.id('Mint(address,address,int24,int24,uint128,uint256,uint256)')
  ) {
    parsedData = abiCoder.decode(
    ...
    event.topics[0] ===
    ethers.utils.id('Burn(address,int24,int24,uint128,uint256,uint256)')
  ) {
    parsedData = abiCoder.decode(
...
export const streamPendingTransactions = (
  wssUrl,
  eventEmitter,
  poolAddress,
) => {
  ...
  const mintEventSignature = ethers.id(
    'Mint(address,address,int24,int24,uint128,uint256,uint256)',
  );
  const burnEventSignature = ethers.id(
